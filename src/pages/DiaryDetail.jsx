...
export default function DiaryDetail() {
  ...
  return (
    <div style={{ maxWidth: '430px', margin: '0 auto', padding: '20px', paddingBottom: '80px' }}>
      ...
      <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '8px' }}>{diary.title}</h2>
      <div style={{ fontSize: '13px', color: '#888', marginBottom: '16px' }}>
        {new Date(diary.createdAt).toLocaleDateString('ko-KR')}
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '20px' }}>
        {diary.emotions?.map(e => (
          <span key={e} style={{ padding: '6px 12px', borderRadius: '20px', background: '#a4c639', color: '#fff', fontSize: '13px', fontWeight: 500 }}>
            #{e}
          </span>
        ))}
      </div>
      <div style={{ background: '#fff', border: '1px solid #ddd', borderRadius: '10px', padding: '16px', fontSize: '15px', lineHeight: '1.7', color: '#333', whiteSpace: 'pre-wrap' }}>
        {diary.content}
      </div>
    </div>
  )
