    ...
    <div style={{ maxWidth: '430px', margin: '0 auto', padding: '20px', paddingBottom: '80px' }}>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <textarea value={content} onChange={e => setContent(e.target.value)}
          placeholder="오늘의 미션을 수행하며 느낀 점을 적어보세요." rows={6}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {EMOTION_OPTIONS.map(emotion => (
            <button key={emotion} onClick={() => toggleEmotion(emotion)}
              style={{ padding: '8px 14px', borderRadius: '20px', border: '1px solid #ddd',
                background: emotions.includes(emotion) ? '#a4c639' : '#fff',
                color: emotions.includes(emotion) ? '#fff' : '#333', cursor: 'pointer', fontWeight: 500 }}>
              {emotion}
            </button>
          ))}
        </div>
